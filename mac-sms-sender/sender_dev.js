...
async function fetchPendingMessages() {
  try {
    ...
    const response = await axios.get(`${CONFIG.WEBSITE_URL}/api/admin/sms/queue`, {
      headers: {
    ...
    if (error.response?.status === 401) {
      ...
      log('❌ Cannot connect to website. Make sure your Next.js app is running.', 'ERROR');
    } else {
...
async function updateMessageStatus(messageId, status, error = null) {
  try {
    ...
    await axios.post(`${CONFIG.WEBSITE_URL}/api/admin/sms/queue`, {
      messageId,
      status,
      error,
    }, {
    ...
    log(`❌ Failed to update message ${messageId} status: ${error.message}`, 'ERROR');
  }
}
...
function performStartupChecks() {
  ...
  // Check configuration
  if (!CONFIG.SECRET_TOKEN || CONFIG.SECRET_TOKEN === 'your-super-secret-key-here') {
    log('❌ Please set SMS_SECRET_TOKEN environment variable', 'ERROR');
    log('   Example: export SMS_SECRET_TOKEN="your-actual-secret-key"', 'ERROR');
    process.exit(1);
  }
  ...
  try {
    execSync('osascript -e "tell application \\"Messages\\" to get name"', { stdio: 'pipe' });
