...
interface BarReservationFormProps {
  minDate: string;
  maxDate: string;
  userId: string;
}

export default function BarReservationForm({
  minDate,
  maxDate,
  userId,
}: BarReservationFormProps) {
  ...
  const maxAvailableDuration = useMemo(() => {
    ...
    try {
      return calculateMaxAvailableDuration(selectedStartTime, slots, partySize, 'bar', date);
    } catch (error) {
  ...
  }, [selectedStartTime, slots, partySize, date]);
  ...
  const getDurationLabel = (hours: number): string => {
    ...
    if (selectedStartTime && hours === maxAvailableDuration && maxAvailableDuration < 8) {
      // Check what's actually limiting the duration, in order of priority
      ...
      } else if (partySize < 3 && hours >= 2) {
        // Check if party size is actually the limiting factor
        // If closing time would allow more than 2 hours, then party size is the limit
        try {
          const closingLimitHours = Math.floor((1560 - timeToMinutes(selectedStartTime, date)) / 60 * 2) / 2; // Convert to 30-min increments
          if (closingLimitHours > 2) {
            return `${baseLabel} (max for party size)`;
          } else {
            return baseLabel;
          }
        } catch (error) {
  ...
  // Get available duration options based on party size and max available duration
  const getAvailableDurations = () => {
    ...
    if (selectedStartTime) {
      ...
      for (let i = 1; i <= maxDurationSlots; i++) {
        ...
        // Apply party size restrictions for bar reservations
        if (partySize < 3 && hours > 2) {
          break;
        }
  ...
  }, [partySize, selectedStartTime, maxAvailableDuration]);
    ...
    const fetchSlots = async () => {
      ...
      try {
        const response = await fetch(
          `/api/reservations/available?date=${date}&type=bar`
        );
  ...
  const handlePartySizeChange = (newPartySize: number) => {
    ...
    // selectedStartTime will be cleared by the useEffect above if needed
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        {partySize < 3 && duration !== null && duration > 2 && (
          <p className="mt-1 text-xs text-red-600">
            Note: Groups of less than 3 people are limited to 2 hours maximum
          </p>
        )}
      </div>
